...
namespace QuanLy_KhachHang.DataAccess
{
	public class NhanVienDA
	{
	    ...
	    public List<NhanVien> GetByTop(string Top, string Where, string Order)
	    {
	        using (
	            IDataReader reader = SqlHelper.ExecuteReader(Data.ConnectionString, CommandType.StoredProcedure,
	                "sproc_NhanVien_GetByTop", Data.CreateParameter("Top", Top)
	                , Data.CreateParameter("Where", Where)
	                , Data.CreateParameter("Order", Order)))
	        {
	            List<NhanVien> list = new List<NhanVien>();
	            while (reader.Read())
	            {
